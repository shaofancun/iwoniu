...
(function(a) {
	a.fn.bsgrid_paging = {
		defaults: {
			...
			pagingBtnClass: "btn btn-default btn-xs"
		},
		pagingObjs: {},
		init: function(h, g) {
			var d = {
		...
		gotoPage: function(b, c) {
			...
			if (a.trim(c) == "" || isNaN(c)) {
				jQuery.jGrowl(a.bsgridLanguage.needInteger)
			} else {
				if (parseInt(c) < 1 || parseInt(c) > b.totalPages) {
					jQuery.jGrowl(a.bsgridLanguage.needRange(1, b.totalPages))
				} else {
	...
	$.fn.bsgrid = {
		...
		page: function(curPage, options) {
			if ($.trim(curPage) == "" || isNaN(curPage)) {
				jQuery.jGrowl($.bsgridLanguage.needInteger);
				return
			}
			...
			$.ajax({
				...
				error: function(XMLHttpRequest, textStatus, errorThrown) {
					jQuery.jGrowl($.bsgridLanguage.errorForSendOrRequestData)
				}
			})
		},
		loadGridData: function(dataType, gridData, options) {
			...
			if (parseSuccess) {
				...
				jQuery.jGrowl($.bsgridLanguage.errorForRequestData)
			}
...
(function(a) {
	...
	a.bsgrid.forcePushPropertyInObject(a.fn.bsgrid.defaults.extend.afterRenderGridMethods, "fixedHeader", function(d, c, b) {
		if (b.settings.extend.settings.fixedGridHeader) {
	...
	a.fn.bsgrid.extendInitGrid.initGridCheck = function(b, c) {
		a.fn.bsgrid.getGridHeaderObject(c).each(function(e) {
			var d = a.trim(a(this).attr(c.settings.colsProperties.checkAttr));
			if (d == "true") {
				...
				a(this).find("input[type=checkbox]").change(function() {
					var f = a.bsgrid.adaptAttrOrProp(a(this), "checked") ? true : false;
					a.bsgrid.adaptAttrOrProp(a("#" + b + " tbody tr td:nth-child(" + (e + 1) + ")>input[type=checkbox]"), "checked", f)
				})
			}
		})
	};
	...
	a.fn.bsgrid.extendInitGrid.initSearchConditions = function(b, c) {
		...
		var f = {};
		a.fn.bsgrid.getGridHeaderObject(c).each(function() {
			var g = a.trim(a(this).attr(c.settings.colsProperties.indexAttr));
	...
	a.fn.bsgrid.extendInitGrid.initColumnMove = function(c, d) {
		if (!d.settings.extend.settings.supportColumnMove || a("#" + d.gridId + " thead tr").length != 1) {
			return
		}
		a("#" + d.gridId).css({
			"table-layout": "fixed"
	...
	a.fn.bsgrid.extendRenderPerColumn.renderCheck = function(e, h, j, d, g, k) {
		if (h < k.curPageRowsNum) {
			var c = a.fn.bsgrid.getGridObj(k.gridId);
			var b = c.getColumnAttr(j, k.settings.colsProperties.checkAttr);
			var f = c.getColumnAttr(j, k.settings.colsProperties.indexAttr);
			var i = c.getColumnValue(h, f);
			if (b == "true") {
				return '<input class="bsgrid_editgrid_check" type="checkbox" value="' + i + '"/>'
			}
		}
		return false
	};
	...
	a.fn.bsgrid.extendRenderPerColumn.renderForm = function(d, g, j, c, f, k) {
		if (k.settings.extend.settings.supportGridEdit && g < k.curPageRowsNum) {
			var b = a.fn.bsgrid.getGridObj(k.gridId);
			var e = b.getColumnAttr(j, k.settings.colsProperties.indexAttr);
			var i = b.getColumnAttr(j, k.settings.colsProperties.editAttr);
			var h = b.getColumnValue(g, e);
			if (i == "checkbox") {
				return h + '<input class="bsgrid_editgrid_hidden bsgrid_editgrid_checkbox" type="' + i + '" value="' + h + '"/>'
			} else {
				if (i == "text" || i == "hidden" || i == "password" || i == "radio" || i == "button") {
					return h + '<input class="bsgrid_editgrid_hidden bsgrid_editgrid_edit" type="' + i + '" value="' + h + '"/>'
				} else {
					if (i == "textarea") {
						return h + '<textarea class="bsgrid_editgrid_hidden bsgrid_editgrid_edit">' + h + "</textarea>"
					}
				}
			}
		}
	...
	a.bsgrid.forcePushPropertyInObject(a.fn.bsgrid.defaults.extend.afterRenderGridMethods, "renderLineNumber", function(d, c, b) {
		a.fn.bsgrid.getGridHeaderObject(b).each(function(f) {
			var e = a.trim(a(this).attr(b.settings.colsProperties.lineNumberAttr));
	...
	a.fn.bsgrid.extendAfterRenderGrid.addCheckChangedEvent = function(d, c, b) {
		a.fn.bsgrid.getGridHeaderObject(b).each(function(f) {
			var e = a.trim(a(this).attr(b.settings.colsProperties.checkAttr));
			if (e == "true") {
				var h = a(this).find("input[type=checkbox]");
				var g = a("#" + b.gridId + " tbody tr td:nth-child(" + (f + 1) + ")>input[type=checkbox]");
				g.change(function() {
					var i = a.bsgrid.adaptAttrOrProp(h, "checked") ? true : false;
					if (!i && g.filter(":checked").length == g.length) {
						a.bsgrid.adaptAttrOrProp(h, "checked", true)
					} else {
						if (i && g.filter(":checked").length != g.length) {
							a.bsgrid.adaptAttrOrProp(h, "checked", false)
						}
					}
				})
			}
	...
	a.fn.bsgrid.extendAfterRenderGrid.addGridEditEvent = function(e, c, b) {
		...
		a("#" + b.gridId + " tbody tr:lt(" + b.curPageRowsNum + ")").each(function(f) {
			...
			a(g).find("td").each(function(i) {
				var l = a.fn.bsgrid.getGridObj(b.gridId);
				var h = l.getColumnAttr(i, b.settings.colsProperties.indexAttr);
				var j = l.getColumnAttr(i, b.settings.colsProperties.editAttr);
				var k = l.getColumnValue(f, h);
				if (j != "") {
					a(this).find(".bsgrid_editgrid_checkbox, .bsgrid_editgrid_edit").change(function() {
						if (a.trim(a(this).val()) != k) {
					...
					if (b.settings.extend.settings.supportGridEditTriggerEvent == "rowDoubleClick") {
						...
						if (b.settings.extend.settings.supportGridEditTriggerEvent == "cellClick") {
							...
							if (b.settings.extend.settings.supportGridEditTriggerEvent == "cellDoubleClick") {
								a(this).find(".bsgrid_editgrid_hidden").each(function() {
									var h = this;
									a(h).parent("td").dblclick(function() {
										d(h)
									})
								})
							}
	...
	a.fn.bsgrid.extendAfterRenderGrid.aggregation = function(f, c, b) {
		var d = b.settings.colsProperties.aggAttr;
		if (a("#" + b.gridId + " tfoot td[" + d + "!='']").length == 0) {
			return
		}
		var e = a.fn.bsgrid.getGridObj(b.gridId);
		a("#" + b.gridId + " tfoot tr td[" + d + "!='']").each(function() {
			var g = a.trim(a(this).attr(d));
			if (g.length != 0) {
				var h = g.split(",");
				var l = h[0].toLocaleLowerCase();
				var j = h.length > 1 ? h[1] : "";
				var k = null;
				if (l == "count") {
					k = b.curPageRowsNum
				} else {
					if (l == "countnotnone" || l == "sum" || l == "avg" || l == "max" || l == "min" || l == "concat") {
						if (l == "countnotnone") {
							k = 0
						}
						var i = new StringBuilder();
						a("#" + b.gridId + " tbody tr:lt(" + b.curPageRowsNum + ")").each(function(m) {
							var n = e.getColumnValue(m, j);
							if (n == "") {} else {
								if (l == "countnotnone") {
									k = (k == null ? 0 : k) + 1
								} else {
									if (l == "sum" || l == "avg") {
										if (!isNaN(n)) {
											k = (k == null ? 0 : k) + parseFloat(n)
										}
									} else {
										if (l == "max" || l == "min") {
											if (!isNaN(n) && (k == null || (l == "max" && parseFloat(n) > k) || (l == "min" && parseFloat(n) < k))) {
												k = parseFloat(n)
											}
										} else {
											if (l == "concat") {
	...
	a.fn.bsgrid.defaults.extend.getRowsChangedColumnsValue = function(c) {
		var b = {};
		var d = a.fn.bsgrid.getGridObj(c.gridId);
		a.each(a.fn.bsgrid.defaults.extend.getChangedRowsIndexs(c), function(e, f) {
			b["row_" + f] = {};
			a("#" + c.gridId + " tbody tr:eq(" + f + ") td").each(function(h) {
				if (a(this).find(".bsgrid_editgrid_change").length > 0) {
					var g = d.getColumnAttr(h, c.settings.colsProperties.indexAttr);
					b["row_" + f][g] = a(this).find(".bsgrid_editgrid_change").val()
				}
			})
		});
		return b
