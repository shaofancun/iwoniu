...
$.fn.rater	= function(options) {
	...
	var settings = {
		...
		title_format	: null,
		info_format	: null,
		image	: '/Content/img/stars.jpg',
		imageAll :'/Content/img/stars-all.gif',
		defaultTips :true,
		clickTips :false,
		width	: 24,
		height	: 24
	...
	//显示结果区域
	var result= $('<div class="rater-star-result"></div>');
	container.after(result); 
	//显示点击提示
	var clickTips= $('<div class="rater-click-tips"><span>点击星星就可以评分了</span></div>');
		if(!settings.clickTips){
			clickTips.hide();	
		}
	container.after(clickTips); 
	//默认手形提示
	var tipsItem= $('<li class="rater-star-item-tips"></li>');
	...
	for (var value=settings.min ; value<=settings.max ; value+=settings.step) {
		item	= $('<li class="rater-star-item"><div class="popinfo"></div></li>');
	...
	content.find('.rater-star-item').mouseover(function() {
		$(this).prevAll('.rater-star-item-tips').hide();
		$(this).attr('class' , 'rater-star-item-hover');
		//$(this).find(".popinfo").show();
		
		//当3分时用笑脸表示
		if(parseInt($(this).css("width"))==shappyWidth){
			$(this).addClass('rater-star-happy');
		}
		//当4分时用笑脸表示
		if(parseInt($(this).css("width"))==happyWidth){
			$(this).addClass('rater-star-happy');
		}
		//当5分时用笑脸表示
		if(parseInt($(this).css("width"))==fullWidth){
			$(this).removeClass('rater-star-item-hover');
			$(this).css('background-image' , 'url(' + settings.imageAll + ')');
			$(this).css({cursor:'pointer',position:'absolute',left:'0',top:'0'});
		}
	}).mouseout(function() {
		var outObj=$(this);
		...
		$(this).prevAll('.rater-star-item-tips').show();
		//var startTip=function () {
		//outObj.prevAll('.rater-star-item-tips').show();
		//};
		//startTip();
	}).click(function() {
		//$(this).prevAll('.rater-star-item-tips').css('display','none');
		$(this).parents(".rater-star").find(".rater-star-item-tips").remove();
		$(this).parents(".goods-comm-stars").find(".rater-click-tips").remove();
		$(this).prevAll('.rater-star-item-current').css('width' , $(this).width());
		   if(parseInt($(this).prevAll('.rater-star-item-current').css("width"))==happyWidth||parseInt($(this).prevAll('.rater-star-item-current').css("width"))==shappyWidth){	
			$(this).prevAll('.rater-star-item-current').addClass('rater-star-happy');
			}
		else{
			$(this).prevAll('.rater-star-item-current').removeClass('rater-star-happy');
			}
			if(parseInt($(this).prevAll('.rater-star-item-current').css("width"))==fullWidth){	
			$(this).prevAll('.rater-star-item-current').addClass('rater-star-full');
			}
		else{
			$(this).prevAll('.rater-star-item-current').removeClass('rater-star-full');
			}
		var star_count		= (settings.max - settings.min) + settings.step;
		var current_number	= $(this).prevAll('.rater-star-item').size()+1;
		var current_value	= settings.min + (current_number - 1) * settings.step;
		
		//显示当前分值
		if (typeof settings.title_format == 'function') {
			$(this).parents().nextAll('.rater-star-result').html(current_value+'分&nbsp;'+settings.title_format(current_value));
		}
		$("#StarNum").val(current_value);
		//$(this).parents().next('.rater-star-result').html(current_value);
		//$(this).unbind('mouseout',startTip)
	})
